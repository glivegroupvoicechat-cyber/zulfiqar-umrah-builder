...
function hotelCalc(row, rate, totalPax = 0) {
  const roomTypeInput = row.querySelector('[data-field="roomType"]');
  const roomsInput = row.querySelector('[data-field="rooms"]');
  const roomsManuallyEntered = row.dataset.roomsManual === 'true';
  const capacityPerRoom = num(roomTypeInput?.value);
  if (totalPax > 0 && capacityPerRoom > 0 && !roomsManuallyEntered) {
    roomsInput.value = Math.ceil(totalPax / capacityPerRoom);
  }
  const rooms = num(roomsInput?.value);
  ...
  const roomRate = num(row.querySelector('[data-field="rate"]').value);
  const totalSar = roomRate * rooms * nights;
  const perPersonNight = capacityPerRoom ? roomRate / capacityPerRoom : 0;
  row.querySelector('.capacity').textContent = capacity;
  row.querySelector('.per-person').textContent = `${sar(perPersonNight)} / person / night`;
  row.querySelector('.hotel-total').textContent = `${sar(totalSar)} · ${pkr(totalSar * rate)}`;
  return { ...Object.fromEntries([...row.querySelectorAll('[data-field]')].map(input => [input.dataset.field, input.value])), capacity, totalSar, nights, perPersonNight, roomRate };
}
function calculate() {
  ensureQuoteDetails();
  ...
  const hotels = eachRow('hotel').map(row => hotelCalc(row, rate, pax));
...
function renderClientQuotation(totals, pax, hotels) {
  ...
  const defaultInclusions = [flights.length ? 'Return air ticket' : '', field('visaRequired').checked ? 'Umrah visa included' : '', hotels.length ? 'Hotel accommodation' : '', transports.length ? 'Ground transport' : '', rowData('ziyarat').some(item => item.service || num(item.rate)) ? 'Ziyarat as selected' : ''].filter(Boolean);
  ...
  const hotelCards = hotels.length ? hotels.map(hotel => `<article class="client-item"><div class="service-icon hotel-icon" aria-hidden="true">&#127976;</div><div><h3>${esc(hotel.name || `${hotel.city} hotel`)}</h3><p>${esc(hotel.city)} · ${esc(roomTypeLabel(hotel.roomType))} · ${esc(String(hotel.nights || 0))} night(s)</p><p class="client-hotel-rate"><strong>Rate:</strong> ${esc(sar(hotel.perPersonNight))} per person / night</p></div></article>`).join('') : '<p class="client-empty">Hotel details to be confirmed.</p>';
  const transportCards = transports.length ? transports.map(item => { const type = normaliseTransportType(item); const icon = type === 'Bus Transport' ? '&#128652;' : '&#128663;'; return `<article class="client-item"><div class="service-icon" aria-hidden="true">${icon}</div><div><h3>${esc(type)}</h3><p>${esc(clientRoute)}</p></div></article>`; }).join('') : '<p class="client-empty">Transport details to be confirmed.</p>';
...
function values() { return { version: 3, fields: Object.fromEntries([...form.querySelectorAll('[name]')].map(input => [input.name, input.type === 'checkbox' ? input.checked : input.value])), rows: Object.fromEntries(Object.keys(lists).map(type => [type, rowData(type)])) }; }
function persist() { try { localStorage.setItem(storageKey, JSON.stringify(values())); } catch { /* Browser storage is optional; JSON export still works. */ } }
function restore(data) { if (!data || !data.fields || !data.rows) throw new Error('This file is not a valid Umrah package.'); Object.entries(data.fields).forEach(([name, value]) => { if (!field(name)) return; field(name)[field(name).type === 'checkbox' ? 'checked' : 'value'] = value; }); Object.keys(lists).forEach(type => { document.querySelector(lists[type]).innerHTML = ''; const savedRows = data.rows[type] || []; savedRows.forEach(value => addRow(type, value)); if (!savedRows.length && (type === 'flight' || type === 'hotel')) addRow(type); }); ensureQuoteDetails(); setMode(field('viewMode').value, false); calculate(); }
function dateTime(value) { const date = value ? new Date(value) : new Date(); return Number.isNaN(date.valueOf()) ? '—' : date.toLocaleString('en-PK', { dateStyle: 'medium', timeStyle: 'short' }); }
...
form.addEventListener('input', event => {
  ...
  if (event.target.matches('[data-field="rooms"]')) event.target.closest('article').dataset.roomsManual = 'true';
  calculate();
});
form.addEventListener('change', event => {
  if (event.target.matches('[data-field="roomType"]')) event.target.closest('article').dataset.roomsManual = 'false';
  calculate();
});
