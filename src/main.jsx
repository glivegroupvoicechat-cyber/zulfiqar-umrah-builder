...
const defaultVoucher = {
  ...
  rows: [
    { id: crypto.randomUUID(), nights: '08', makkahName: 'HIBA HIJRA 6 OR SIMILAR', makkahDistance: 'Shuttle', madinahName: 'RETAJ AL MADINA OR SIMILAR', madinahDistance: 'Shuttle', sharing: '268,600', quad: '273,700', triple: '282,000', double: '298,700' },
    { id: crypto.randomUUID(), nights: '09', makkahName: 'HIBA HIJRA 6 OR SIMILAR', makkahDistance: 'Shuttle', madinahName: 'SAFEER SAKNI 2 OR SIMILAR', madinahDistance: '700M', sharing: '275,900', quad: '280,500', triple: '291,100', double: '312,400' },
    { id: crypto.randomUUID(), nights: '10', makkahName: 'JADA KHALIL OR SIMILAR', makkahDistance: '1200M', madinahName: 'KINAN OR SIMILAR', madinahDistance: '900M', sharing: '273,600', quad: '280,300', triple: '289,700', double: '311,000' },
  ],
};
...
function VoucherForm({ data, setData }) {
  ...
  const addRow = () => setData({ ...data, rows: [...data.rows, { id: crypto.randomUUID(), nights: '', makkahName: '', makkahDistance: '', madinahName: '', madinahDistance: '', sharing: '', quad: '', triple: '', double: '' }] });
  return (
    <>
  ...
  return (
    <article className={`brochure brochure-${data.style}`}>
      ...
      <div className="hotel-duo">
        <InfoCard icon={<Hotel />} title="Makkah Hotel" main={data.hotels.makkahName} sub={data.hotels.makkahDistance} />
        <InfoCard icon={<MapPin />} title="Madinah Hotel" main={data.hotels.madinahName} sub={data.hotels.madinahDistance} />
      </div>
...
function VoucherPreview({ data }) {
  return (
    <article className={`brochure brochure-${data.style} rate-sheet`}>
        ...
        {data.rows.map((row, index) => (
          <section className="rate-item" key={row.id}>
            ...
            <div className="hotel-pair">
              <div><span>Makkah Hotel</span><strong>{row.makkahName || 'Makkah Hotel'}</strong><p>{row.makkahDistance}</p></div>
              <div><span>Flight Schedule</span><p>{data.flights.outbound}</p><p>{data.flights.inbound}</p></div>
              <div><span>Madinah Hotel</span><strong>{row.madinahName || 'Madinah Hotel'}</strong><p>{row.madinahDistance}</p></div>
            </div>
      ...
      <footer className="preview-footer"><div><strong>{data.branding.name}</strong><p>{data.branding.address}</p></div><div className="qr-placeholder">QR</div></footer>
    </article>
  );
}
...
function InfoCard({ icon, title, main, sub }) {
  return <div className="info-card">{icon}<div><span>{title}</span><strong>{main}</strong><p>{sub}</p></div></div>;
}
